---
name: "source-command-selector-regression"
description: "Triage a host-site selector regression (ChatGPT / Codex redesign broke injection)."
---

# source-command-selector-regression

Use this skill when the user asks to run the migrated source command `selector-regression`.

## Command Template

Triage a host-site selector regression. The first argument is the host (`chatgpt` or `Codex`); the optional second argument is a replacement selector the user has already identified.

## Steps

1. Identify the relevant content script:
   - `chatgpt` → `scripts/gpt-content.js`
   - `Codex` → `scripts/Codex-content.js`

2. Find the current injection selector in that file. Common anchors:
   - ChatGPT: `#conversation-header-actions`
   - Codex: `header .right-3.flex.gap-2 > div`
