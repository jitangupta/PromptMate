...
(function initDeepSeekPromptMate() {
  ...
  function createSidebar() {
    ...
    // DeepSeek's React tree, a transformed ancestor would otherwise become the
  ...
  // ────────────────────────────────────────────────────────────
  // Compose + Use (DeepSeek chat input)
  // ────────────────────────────────────────────────────────────
  ...
  function onUse(prompt) {
    ...
    const result = insertIntoDeepSeekInput(text);
    if (!result.success) {
  ...
  // createPill is idempotent — keep observing body in case the host clears
  // its children during heavy SPA transitions.
